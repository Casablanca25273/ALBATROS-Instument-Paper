...
% caught brief glimpses of the $\sim 2$~MHz sky at low
% resolution~\citep{reber}.  The lowest frequency to date for
% high-resolution sky imaging has been achieved by the LWA, which
% reported observations at 36.5~MHz using only 28 hours of
% data~\citep{lwa}.  Preliminary observations from Marion Island
% (\S\ref{s:dark_method}) suggest that despite the present-day RFI
% environment, low-frequency observations may still be accessible from
% carefully selected locations and with new technology developments.
