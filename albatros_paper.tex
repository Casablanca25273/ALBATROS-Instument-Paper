...
Model (GSM)~\citep{gsm2008} at only the $\sim$10\% level.  Additionally, as
...
fluctuations between 30 and 40~MHz (where the Galactic signal peaks in the
data) are less than 1\%.  Further, we find that the typical noise on
$\sim$3-second timescales is 0.04\% of the power.  The low noise suggests that we
can carry out relative calibration with sub-percent accuracy by using
the autospectra recorded by each autonomous station (absolute calibration
could be carried out by comparison to either \prizm\, which is
co-located and absolutely calibrated, or \textit{e.g.} the GSM).
As a qualitative illustration of the smaller-scale sky signal repeatability,
